 ...
 * Masrik Dahir
 * Date: 02/20/2022
 * This program runs on the client side and get the ascending or 
 * descending of the input from the server 
 *
 * 
...
void averageprog_1( char* host, int argc, char *argv[] )
{
   ...
   if (strcmp(argv[2],"-a") == 0) { 
      ...
      printf("Missing -a (Ascending) or -d (Descending)\n");
    	exit(2);
   }

   average_1_arg.input_data.input_data_val = (double*) malloc(MAXAVGSIZE*sizeof(double));
   result.input_data.input_data_val = (double*) malloc(MAXAVGSIZE*sizeof(double));
   ...
   *dp = option;
   ...
   result = *average_1( &average_1_arg, clnt);
   clnt_destroy( clnt );

   double *pp;
   pp = result.input_data.input_data_val;
   pp++;
