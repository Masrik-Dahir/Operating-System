 ...
 *
 * Usage Guide
 * (1) gcc -g FIFO.c -o FIFO.exe -lpthread
 * (2) ./FIFO.exe
 */
...
double time_in_between(struct timespec *start, struct timespec *end) {
...
typedef struct shm_struct {
    ...
    //buffer array
    print_job buffer[BUFFER_SIZE];

    //top element in the stack buffer
    int firstElement;
    int firstOpenSpot;
    int storage;
...
// insertign elements in the buffer
void insertbuffer(print_job value, shm_struct* shm) {
    if(shm->storage < BUFFER_SIZE) {
        shm->buffer[shm->firstOpenSpot++ % BUFFER_SIZE] = value;
        shm->storage++;
    }
    else
...
// dequeueing elements from the buffer
print_job dequeuebuffer(shm_struct* shm) {
    if(shm->storage > 0) {
        shm->storage--;
        return shm->buffer[shm->firstElement++ % BUFFER_SIZE];
    }
    else {
...
void producer() {
    ...
    for(i = 0; i < num_of_jobs; i++) {
        ...
        printf("Producer %u added %d to buffer\n", getpid(), bytesSize);
...
void *consumer(void *thread_n) {
    ...
    while(ref->producersEnd < ref->numOfProducers || ref->printJobProcessed < ref->printJobMade) {
        ...
        printf("Consumer %d dequeue (%u, %d) from buffer\n", thread, val.pid, val.size);
...
void sig_handler(int signum) {
    ...
    if(getpid() != parentPid) {
        ...
        int i, cancelThread;
        for(i = 0; i < numOfThreads; i++)
            cancelThread = pthread_cancel(threads[i]);
...
int main(int argc, char *argv[]) {
    ...
    if(argc == 3) {
        ...
        printf("Correct argument: %s %s %s", argv[0], "<processes (int)>", "<threads (int)>\n");
        exit(1);
    }
    ...
    if(parentPid == getpid()) {
        ...
        for(thread = 0; thread < numOfThreads; thread++) {
            pthread_create(consumer_threads + thread, NULL, consumer, &thread); //consumer_threads stores p_thread
        }
